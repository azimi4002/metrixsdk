...
- (NSMutableDictionary *)getSessionStartParameters:(BOOL)isInDelay {
    ...
    NSString *engineName = @"ios";
#ifdef REACT_NATIVE
    engineName = @"reactnative";
#endif

    [MXPackageBuilder parameters:parameters setDictionary:@{@"package":self.deviceInfo.bundeIdentifier,
            @"version":self.deviceInfo.bundleVersion,
            @"engine_name": engineName} forKey:@"app_info"];

    [MXPackageBuilder parameters:parameters setDictionary:
            @{@"install_complete_timestamp":@([MXUtil getInstallTimestamp]),
            @"update_timestamp":@([MXUtil getUpdateTimestampNormalized])} forKey:@"install_info"];

    if(self.metrixConfig.trackerToken){
        [MXPackageBuilder parameters:parameters setDictionary:
                [self attributesDictionary:@{@"tracker_token":self.metrixConfig.trackerToken}] forKey:@"attributes"];
    }
    ...
    [MXPackageBuilder parameters:parameters setDictionary:@{@"idfa": [MXUtil idfa], @"cpu_abi":self.deviceInfo.cpuSubtype,
            ...
            @"language":self.deviceInfo.languageCode, @"os":os ,@"screen":screen,
            @"facebook_attribution_id": self.deviceInfo.fbAttributionId} forKey:@"device_info"];

    [MXPackageBuilder parameters:parameters setDictionary:[MXUtil locationDictionary] forKey:@"geo_info"];
    ...
    if([MXUtil readReachabilityFlags])connectionInfo[@"connection_type"] = [MXUtil readConnectionType];
    ...
    if (self.activityState != nil) {
        ...
        [MXPackageBuilder parameters:parameters setInt:self.activityState.sessionCount-1 forKey:@"session_num"];
...
- (NSMutableDictionary *)getSessionStopParameters {
    ...
    [MXPackageBuilder parameters:parameters setString:[MXUtil formatSeconds1970:self.createdAt] forKey:@"event_time"];
    [MXPackageBuilder parameters:parameters setDuration:self.createdAt forKey:@"event_time_stamp"];
    [MXPackageBuilder parameters:parameters setString:self.activityState.sessionId forKey:@"session_id"];
    [MXPackageBuilder parameters:parameters setString:self.activityState.userId forKey:@"user_id"];
    ...
    if (self.activityState != nil) {
        [MXPackageBuilder parameters:parameters setNumber:@((long)self.activityState.sessionLength*1000) forKey:@"duration_millis"];
        [MXPackageBuilder parameters:parameters setArray:[self.activityState getCompleteScreenFlow] forKey:@"screen_flows"];
        [MXPackageBuilder parameters:parameters setDictionary:[self attributesDictionary:nil] forKey:@"attributes"];
    }
...
- (NSMutableDictionary *)getCustomEventParameters:(BOOL)isInDelay forEventPackage:(MXCustomEvent *)event {
    ...
    [MXPackageBuilder parameters:parameters setDictionary:[self attributesDictionary:event.attributes] forKey:@"attributes"];
...
- (NSDictionary *)attributesDictionary:(NSDictionary *)eventDic {
    NSMutableDictionary * toReturn = [NSMutableDictionary dictionary];
    if(eventDic != nil)[toReturn addEntriesFromDictionary:eventDic];
    if(self.activityState.attributes != nil)[toReturn addEntriesFromDictionary:self.activityState.attributes];
    return toReturn;
}
...
- (BOOL)needInitRequest{
    return !self.activityState || self.activityState.sessionCount <= 1;
}

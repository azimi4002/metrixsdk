...
- (void)sendFirstI:(MXPackageHandler *)selfI
{
    ...
    if (![activityPackage.path isEqualToString:BASE_PATH_INIT] && !activityPackage.parameters[@"user_id"]) {
        [selfI.logger verbose:@"userId is not still available."];
        [selfI delayThisI:selfI];
        return;
    }
...
- (void)delayThisI:(MXPackageHandler *)selfI {
    if ([selfI.packageQueue count] > 0) {
        id obj = [selfI.packageQueue objectAtIndex:0];
        [selfI.packageQueue removeObjectAtIndex:0];
        [selfI.packageQueue addObject:obj];
        [selfI writePackageQueueS:selfI];
    }
...
//    [selfI.logger verbose:@"Session partner parameters: %@", sessionParameters.partnerParameters];
    ...
    for (MXActivityPackage * activityPackage in selfI.packageQueue) {
        if(!activityPackage.parameters[@"user_id"] && sessionParameters.userId){
            [MXPackageBuilder parameters:activityPackage.parameters
                               setString:sessionParameters.userId
                                  forKey:@"user_id"];
        }

        NSDictionary * mergedAttributes = [MXUtil mergeParameters:activityPackage.parameters[@"attributes"]
                                                                   source:sessionParameters.attributes
                                                            parameterName:@"attributes"];

        [MXPackageBuilder parameters:activityPackage.parameters
                        setDictionary:mergedAttributes
                               forKey:@"attributes"];
    }

...
+ (MXEventFailure *)eventFailureResponseData {
    return [[MXEventFailure alloc] init];
}

#pragma mark - NSCopying protocol methods

- (id)copyWithZone:(NSZone *)zone {
    MXEventFailure *copy = [[[self class] allocWithZone:zone] init];
...
- (NSString *)description {
    return [NSString stringWithFormat: @"Event Failure msg:%@ time:%@ mxid:%@ event:%@ cid:%@, retry:%@ json:%@",
            self.message,
            self.timeStamp,
            self.adid,
            self.eventToken,

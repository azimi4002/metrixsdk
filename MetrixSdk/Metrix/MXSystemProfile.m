...
@implementation MXSystemProfile
...
+ (NSString*) cpuFamily
{
    ...
    if (error != 0) {
        [MXMetrixFactory.logger error:@"Failed to obtain CPU family (%d)", error];
        return nil;
    }
    switch (cpufamily)
    {
    ...
    [MXMetrixFactory.logger warn:@"%@", unknowCpuFamily];
...
+ (int) cpuCount
{
    ...
    if (error != 0) {
        [MXMetrixFactory.logger error:@"Failed to obtain CPU count (%d)", error];
        return 1;
    }
...
{
    return [MXSystemProfile readSysctlbByNameString:"hw.machinearch" errorLog:@"Failed to obtain machine arch"];
}

+ (NSString*) machineModel
{
    return [MXSystemProfile readSysctlbByNameString:"hw.model" errorLog:@"Failed to obtain machine model"];
}

+ (NSString*) cpuBrand
{
    return [MXSystemProfile readSysctlbByNameString:"machdep.cpu.brand_string" errorLog:@"Failed to obtain CPU brand"];
}

+ (NSString*) cpuFeatures
{
    return [MXSystemProfile readSysctlbByNameString:"machdep.cpu.features" errorLog:@"Failed to obtain CPU features"];
}

+ (NSString*) cpuVendor
{
    return [MXSystemProfile readSysctlbByNameString:"machdep.cpu.vendor" errorLog:@"Failed to obtain CPU vendor"];
}

+ (NSString*) osVersion
{
    return [MXSystemProfile readSysctlbByNameString:"kern.osversion" errorLog:@"Failed to obtain OS version"];
}
                             ...
                             errorLog:(NSString*)errorLog
{
    ...
    if (error != 0) {
        [MXMetrixFactory.logger error:@"%@ (%d)", errorLog, error];
        return nil;
    }
...
+ (NSString*) appleLanguage
{
    ...
    if ([languages count] == 0) {
        [MXMetrixFactory.logger error:@"Failed to obtain preferred language"];
        return nil;
    }
...
+ (long long) cpuSpeed
{
    ...
    if (error) {
        [MXMetrixFactory.logger error:@"Failed to obtain CPU speed (%d)", error];
        return -1;
    }
...
+ (long long) ramsize
{
	...
	if (error) {
        [MXMetrixFactory.logger error:@"Failed to obtain RAM size (%d)", error];
        return -1;
	}
...
+ (NSString*) cpuType
{
    ...
    if (error != 0) {
        [MXMetrixFactory.logger error:@"Failed to obtain CPU type (%d)", error];
        return nil;
    }

    NSString * cpuTypeString = [MXSystemProfile readCpuTypeSubtype:cputype readSubType:NO cpusubtype:0];
    ...
    [MXMetrixFactory.logger warn:@"%@", unknowCpuType];
...
+ (NSString*) cpuSubtype
{
    ...
    if (error != 0) {
        [MXMetrixFactory.logger error:@"Failed to obtain CPU subtype (%d)", error];
        return nil;
    }


    NSString * cpuSubtypeString = [MXSystemProfile readCpuTypeSubtype:cputype readSubType:YES cpusubtype:cpuSubtype];
    ...
    [MXMetrixFactory.logger warn:@"%@", unknowCpuSubtype];

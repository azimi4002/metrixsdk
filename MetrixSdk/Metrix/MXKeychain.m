...
+ (id)getInstance {
    static MXKeychain *defaultInstance = nil;
...
+ (BOOL)setValue:(NSString *)value forKeychainKey:(NSString *)key inService:(NSString *)service {
    ...
    return [[MXKeychain getInstance] setValue:value forKeychainKey:key inService:service];
}
...
+ (NSString *)valueForKeychainKeyV1:(NSString *)key service:(NSString *)service {
    ...
    return [[MXKeychain getInstance] valueForKeychainKeyV1:key service:service];
}
...
+ (NSString *)valueForKeychainKeyV2:(NSString *)key service:(NSString *)service {
    ...
    return [[MXKeychain getInstance] valueForKeychainKeyV2:key service:service];
}
...
- (BOOL)setValue:(NSString *)value forKeychainKey:(NSString *)key inService:(NSString *)service {
    ...
    if (status != noErr) {
        [[MXMetrixFactory logger] warn:@"Value unsuccessfully written to the keychain v1 way"];
        ...
        if (wasSuccessful) {
            [[MXMetrixFactory logger] warn:@"Value successfully written in v1 way to the keychain after the check"];
        }
...
- (NSMutableDictionary *)keychainItemForKeyV2:(NSString *)key service:(NSString *)service {
    ...
    CFStringRef *cStringSecAttrAccessGroupToken = [MXKeychain getSecAttrAccessGroupToken];
